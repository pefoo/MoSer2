...
class DataColumn : public Column {
  static_assert(!std::is_pointer<ColumnType>::value,
 ...
 public:
  ...
  ///
  explicit DataColumn(std::string name, std::vector<ColumnType> data = {})
  ...
  ///
  [[nodiscard]] size_t size() const override { return this->values_.size(); }

  ///
  /// \copydoc utility::datastructure::Column::ElementAtToString
  ///
  [[nodiscard]] std::string ElementAtToString(
  ...
  ///
  [[nodiscard]] Column* Clone() const override {
    return new DataColumn<ColumnType>{*this};
  }

 private:
  [[nodiscard]] inline std::string StringConverter(const size_t index) const;
  std::vector<ColumnType> values_;
};

template <typename ColumnType>
inline std::string DataColumn<ColumnType>::StringConverter(
    const size_t index) const {
  return std::to_string(this->values_.at(index));
}
template <>
inline std::string DataColumn<std::string>::StringConverter(
    const size_t index) const {
